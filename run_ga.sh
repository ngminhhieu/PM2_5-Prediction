...
NAME_WINDOW=( "pop_40" "pop_50" "pop_60" "pop_70" )
index=(0 1 2 3)
population=(40 50 60 70)
for i in "${index[@]}"
do
    ## Create the windows on which each node or .launch file is going to run
    tmux send-keys -t real "tmux new-window -n ${NAME_WINDOW[$i]} " ENTER
    sleep 1
    ## Send the command to each window from window 0
    tmux send-keys -t real "tmux send-keys -t ${NAME_WINDOW[$i]} 'python main.py --pc=0.8 --pm=0.2 --population=${population[$i]}' ENTER" ENTER
done

NAME_WINDOW=("pc_2" "pc_3" "pc_4" "pc_5" "pc_6" "pc_7" "pc_8")
index=(0 1 2 3 4 5 6)
...
do
    ...
    sleep 1
    ## Send the command to each window from window 0
    tmux send-keys -t real "tmux send-keys -t ${NAME_WINDOW[$i]} 'python main.py --pc=${pc[$i]} --pm=0.2 --population=30' ENTER" ENTER
done
